...
def main() -> int:
    ...
    ap.add_argument("--adapter", default="artifacts/lora_weights/final")
    ...
    print(f"[env] device={device}  base={args.base}  adapter={args.adapter}")
    ...
    print("[post] attaching LoRA adapter")
    model = PeftModel.from_pretrained(base, str(ROOT / args.adapter))

    print(f"[post] measuring m2 on {len(pairs)} fixtures")
    post_rows = measure(model, tokenizer, pairs, label="post")
    ...
    for pre, post in zip(pre_rows, post_rows):
        ...
        combined.append({
            ...
            "post": post,
            "delta_m2": delta_m2,
            "delta_csl_nll": delta_csl_nll,
            "delta_en_nll":  delta_en_nll,
        })
